...
function player:GetData(userid)
	if not player.connections[userid] then
		player.connections[userid] = {
			activity = nil,
			stamina = 100, legs	= 0, arms 	= 0, 
			back 	= 0, cash  = 0, chest = 0,
			rank = 1, level = 0, levelProgress = 0,
			chests = {vip=tick()-60*60*24 + 50, daily=tick()-60*60*24 + 15, group=tick() },
			multiplier = 1,
			clicks = { tick(),tick(),tick(),tick(),tick(),tick(),tick(),tick(),tick(), tick() },
			selected = '',
			backpack = {
				'greenraptor','lightblue','yellow','dactyl','dactly','yasmin','buzz','orange',
				'gray','lean','green','tyson','tom','brown','otheryasmin','flea','frog','rot',
			},
			muscles = {
				lefthamstring = {mass = 0}, leftshoulder = {mass = 0}, leftforearm = {mass = 0}, lefttricep = {mass = 0},
				leftchest = {mass = 0}, leftbicep = {mass = 0}, leftcalve = {mass = 0}, leftneck = {mass = 0},leftquad = {mass = 0},
				torso = {mass = 0 }, abs   = {mass = 0 }, back  = {mass = 0 }, chest = {mass = 0 },
				rightquad = {mass = 0}, rightneck = {mass = 0}, rightchest = {mass = 0}, rightcalve = {mass = 0},
				rightbicep = {mass = 0}, righttricep = {mass = 0}, rightforearm = {mass = 0}, rightshoulder = {mass = 0},righthamstring = {mass = 0},
			},
			auras = {'cloudblue', 'cloudblackpink', 'star', 'xsparkyellow'},
			equipped = '',
			tamer = {
				legs  = 0, chest = 0,
				arms  = 0, stamina = 0, back = 0,
			},
			staminaHealth = 100,
			staminaHealthDebounce = tick(), communicationDebounce = tick(),
			threads = {
				cpschecker = task.spawn(function()
					task.wait(0.555) 
					while task.wait(0.2) do
						player.connections[userid].multiplier = math.clamp(player.connections[userid].multiplier - 0.1, 1, 2)
					end
				end),
				staminaHealther = run.Stepped:Connect(function()
					local t = tick() 
					if t - player.connections[userid].staminaHealthDebounce < 3 then return end
					if player.connections[userid].activity and (t - player.connections[userid].communicationDebounce > 1)  then
						player.connections[userid].staminaHealth = math.min(
							player.connections[userid].stamina,
							player.connections[userid].staminaHealth 
								+ (player.connections[userid].stamina*0.05 + 15)
							--		*task.wait()
						) 
						player:sendUpdate(userid)
						player.connections[userid].communicationDebounce = t
...
function player:updateMuscle(uid:number,what:string, amount:number)
	...
	for i,v in pairs(sacred.gym.rewards[what].muscles) do 
		...
		if (maxSize.Magnitude < currentSize.Magnitude) then continue end
		local sizeGoal = {Size = maxSize*((p.tamer[p.activity.name]/sacred.tamer[sacred.ranks[p.rank + 1]][p.activity.name]))}
...
function player:AttachMuscle(uid:number,att:Attachment&{Parent:Part&{Parent:Part}})
	local p = player.connections[uid]
	local mass = p.muscles[att.Name].mass
	...
	part.CanQuery = false part.CanTouch = false part.CanCollide = false part.Massless = true
	part.CFrame = att.WorldCFrame weldcs.Part0 = att.Parent weldcs.Part1 = part
	att.Parent.Color = part.Color att.Parent.BrickColor = part.BrickColor  
	att.Parent.Material = part.Material att.Parent.MaterialVariant = part.MaterialVariant
	part.Size = Vector3.zero weldcs.Parent = att  part.Parent = att

	local weldcs0  = Instance.new('WeldConstraint') local glowPart:MeshPart = part:Clone()
	glowPart.CFrame = att.WorldCFrame weldcs0.Part0 = att.Parent weldcs0.Part1 = glowPart
	weldcs0.Parent = weldcs glowPart.Parent = att glowPart.Size = part.Size*1.1
	glowPart.Transparency = 1 glowPart.Material = Enum.Material.Neon glowPart.Color = Color3.new(1,1,1)
	glowPart.MaterialVariant = ''
	local transparencyValue = Instance.new("NumberValue")
	transparencyValue:GetPropertyChangedSignal('Value'):Connect(function() glowPart.Transparency = transparencyValue.Value end)
	
	if not p.muscles[att.Name].glow then	p.muscles[att.Name].glow = {} end
	...
	character.Head.Color = part.Color character.Head.BrickColor = part.BrickColor  
	character.Head.Material = part.Material character.Head.MaterialVariant = part.MaterialVariant
end
...
function player:RankUp(uid:number) 
	local p = player.connections[uid]
	local o = p.rank; p.rank = math.min(p.rank + 1 ,#sacred.ranks)
	if o == p.rank then return end
	player:ClearAllMuscles(uid)
	p.tamer = {back = 0,chest = 0, arms = 0,legs = 0, stamina = 0}
	player:setUpMuscles(uid)
	player:sendUpdate(uid) 
	local prtclz =  script.RankUpParticles:Clone()
...
function player:updateTamer(uid:number,amount:number)
	...
	if p.rank >= #sacred.ranks then return end 
...
function player:click(uid)
	...
	if p.activity and p.activity.family == 'gym' then p.staminaHealthDebounce = t end
end 
...
function player:updateCPS(uid)
	...
	local sum = 0
	for i=1,9  do sum += clicks[i+1] - clicks[i] end
...
function player:AddPetToBackPack(userid,petname) table.insert(player.connections[userid].backpack,petname) end
function player:BossWin(playerid)
	bossRemote:FireClient(players:GetPlayerByUserId(playerid),'win')
	local character = players:GetPlayerByUserId(playerid).Character
	local tw = tws:Create(character.PrimaryPart,TweenInfo.new(0.2),{CFrame = CFrame.new(character:GetPivot().Position)})
	tw:Play()
	task.wait(1.2)
	character.PrimaryPart.Anchored = false
	player.connections[playerid].cash += 1000
