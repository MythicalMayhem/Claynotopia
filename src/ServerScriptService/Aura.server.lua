...
auraRemote.OnServerEvent:Connect(function(player, what, which)
	if what == "attach" then
		if table.find(playerManager.connections[player.UserId].auras, which) == nil then
			return
		end
		...
		clo.Parent = att
		--  clo.CFrame *= CFrame.new(0,0.5,1)
		playerManager.connections[player.UserId].equipped = which
